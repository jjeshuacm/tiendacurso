...
export const Cart = () => {
  ...
  //estado para los datos del formulario con onchage
  ...
  const [ordenid,setOrdenId] = useState("");
      ...
      //llenar el estado con los obchange
    ...
    const realizarCompra = async (datos,e) => { 
      ...
      orden.buyer = {name: datos.name, lastname: datos.lastname, email: datos.email, phone: datos.phone}
      // console.log(orden);
      //OBTENER TOTAL
          ...
          {!ordenfine  ?   
            <Form   onSubmit={handleSubmit(realizarCompra)}>
                <Row className=" mb-4 header-background"  >
                      <Col ><h1 className="font-link text-white">Orden de compra:</h1></Col>
                </Row>
                <Row >
                  <Col lg="6" className="mt-4">
                                <Form.Group className="mr-2">
                                    <Form.Label className="mr-2">Nombre</Form.Label>
                                    <Form.Control type="text"  name="name"  placeholder="Nombre" maxLength="10"   
                                    // onChange={ formData }
                                    {...register("name", 
                                        {required: 
                                            {value: true, message: "El campo es requerido"},
                                        pattern: 
                                            {value: /^[a-zA-Z]+$/, message: "El formato no es correcto: solo letras"},
                                        minLength: 
                                            {value: 10,  message: "Debe tener al menos 10 caracteres" }
                                      })}
                                    ></Form.Control>
                                ...
                                <Form.Group className="mr-2">
                                    ...
                                    <Form.Control type="text" name="lastname" placeholder="Apellido" maxLength="10"  
                                    //  onChange={ formData }
                                      {...register("lastname", 
                                      {required: 
                                          {value: true, message: "El campo es requerido"},
                                ...
                                <Form.Group className="mr-2">
                                    ...
                                    <Form.Control type="text" name="phone" placeholder="Telefono"  maxLength="9" 
                                    // onChange={ formData }
                                      {...register("phone", 
                                      {required: 
                                          {value: true, message: "El campo es requerido"},
                                      pattern: 
                                          {value: /^[0-9]+$/, message: "El formato no es correcto: solo números"},
                                      minLength: 
                                          {value: 9,  message: "Debe tener al menos 9 dígitos" }
                                    })}
                                    ></Form.Control>
                                ...
                                <Form.Group className="mr-2">
                                    ...
                                    <Form.Control type="email" name="email" placeholder="Email" maxLength="20"  
                                    // onChange={ formData }
                                      {...register("email", 
                                      {required: 
                                          {value: true, message: "El campo es requerido"},
                                      pattern: 
                                          {value: /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i, message: "El formato no es correcto"},
                                ...
                                <Form.Group className="mr-2">
                                    ...
                                    <Form.Control type="email" name="repitemail" placeholder="Repetir Email" maxLength="20"  
                                    // onChange={ formData }
                                    // metodo getvalue para obtener el valores actual de todo el formulario en este caso email
                                      ...
                                      {required: 
                                          {value: true, message: "El campo es requerido"},
                                      validate: 
                                          {emailEqual: value => (value === getValues().email) || 'No coinciden los email', }
                                    
                                    })}
                                    ></Form.Control>
                                    {errors.repitemail &&  <span className="text-danger">{errors.repitemail.message}</span>}
                  ...
                  <Col lg="6">
                                              ...
                                              <>
                                                      {cartList.map((ele,i) => (
                                                            <ListGroup.Item key={i} id={ele.item.id}>
                                                                    <img className="img-or"   src={ele.item.imagenUrl} alt={ele.item.name}  title={ele.item.name}/>
                                                                    Nombre:  {ele.item.name} /
                                                                    Precio: $ {ele.item.price} / Cantidad: {ele.quantity} /
                                                                    <Button  className='mr-2 ml-3 btn-purple'  onClick={() => removeItem(ele.item.id)}>Eliminar Item</Button>
                                                            </ListGroup.Item>
                                                          ))
                                                        }
                      ...
                      <Row className="mt-4" >
                              <Col >
                                    <Button className='mr-2 btn-purple'  onClick={clear}>Vaciar Carrito </Button>
                                    <Link to='/'>
                                        <Button className="seguircomprando ml-2  mr-2 btn-purple">Seguir Comprando</Button>
                                    </Link>
                                    <Button className='mr-2 ml-2 btn-purple' type="submit"  >Crear Orden </Button>
                              </Col>
                      </Row>
                  </Col>
                </Row>
